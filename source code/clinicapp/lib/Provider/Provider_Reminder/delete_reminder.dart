...
class DeleteReminderProvider extends ChangeNotifier {
  ...
  ///Add task method
  void deleteReminder({int? reminderId, required BuildContext ctx}) async {
    final token = await DatabaseProvider().getToken();
    _status = true;
    notifyListeners();

    final _url = "$url/reminder/$reminderId";
    ...
    if (result.statusCode == 200 || result.statusCode == 201) {
      ...
      notifyListeners();
      PageNavigator(ctx: ctx).nextPageOnly(page: const ReminderPage());
    } else {
      final res = result.body;
      print(res);
      _status = false;
